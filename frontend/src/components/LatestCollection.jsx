...
const LatestCollection = () => {
  ...
  // Center Start
  useEffect(() => {
    if (latestProducts.length > 0 && scrollRef.current) {
      const middleIndex = Math.floor(latestProducts.length / 2);
      setTimeout(() => {
    ...
    <section className='relative w-full'>
      <div className='text-center py-4'>
        ...
        <p className='w-3/4 m-auto text-xs sm:text-sm md:text-base text-gray-600 font-medium'>
          Fresh from the studio. Explore our newest handcrafted additions.
        </p>
      </div>
      ...
      >
        {latestProducts.map((item, index) => {
          ...
          return (
            <div
              ...
              `}
            >
              <div className={`transition-all duration-700 rounded-3xl ${isActive ? 'shadow-[0_20px_50px_-12px_rgba(236,72,153,0.5)]' : 'shadow-none'}`}>
                <ProductItem 
                  id={item._id} 
                  image={item.image} 
                  name={item.name} 
        ...
        <div onClick={() => navigate('/collection/latest')} className={`min-w-[160px] sm:min-w-[200px] snap-center flex-shrink-0 flex flex-col items-center justify-center group cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${activeIndex === latestProducts.length ? 'scale-110 opacity-100' : 'scale-90 opacity-50 blur-[1px]'}`}>
          ...
          <p className="mt-4 text-sm font-bold text-pink-900/70 uppercase tracking-widest group-hover:text-pink-600 transition-colors">View All</p>
        </div>
      </div>
    </section>
  );
